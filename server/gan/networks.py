...
class Generator(tf.keras.Model):
    ...
    def architecture_init(self):
        ...
        for i in range(self.repeat_num):
            ...
            decoder.insert(0, AdainResBlock(ch_out, ch_in, upsample=True, sn=self.sn,
        ...
        for i in range(2):
            ...
            decoder.insert(0,
                           AdainResBlock(ch_out, ch_out, sn=self.sn, name='decoder_bottleneck_adaresblock_' + str(i)))
...
class MappingNetwork(tf.keras.Model):
    ...
    def architecture_init(self):
        ...
        for n_d in range(self.num_domains):
            for i in range(3):
                layers += [
                    FullyConnected(units=self.hidden_dim, sn=self.sn, name='domain_{}_unshared_fc_{}'.format(n_d, i))]
                layers += [Relu(name='relu{}_{}'.format(n_d, i))]
...
class StyleEncoder(tf.keras.Model):
    ...
    def architecture_init(self):
        ...
        blocks += [Leaky_Relu(alpha=0.2, name="lll")]
        blocks += [Conv(channels=ch_out, kernel=4, stride=1, pad=0, sn=self.sn, name='conv')]
        blocks += [Leaky_Relu(alpha=0.2, name="333d")]
...
class Discriminator(tf.keras.Model):
    ...
    def architecture_init(self):
        ...
        blocks += [Leaky_Relu(alpha=0.2, name="akodsi")]
        blocks += [Conv(channels=ch_out, kernel=4, stride=1, pad=0, sn=self.sn, name='conv_0')]

        blocks += [Leaky_Relu(alpha=0.2, name="kdsoi")]
